...
function show_lti_edit_table_cmd($cmds, $params)
{
    ...
    $table->head [] = ''; // get_string('command_options', 'mod_ltids'); 
    $table->align[] = 'left';
    $table->size [] = '50px';
    $table->wrap [] = 'nowrap';
...
function show_lti_edit_table_vol($cmds)
{
    ...
    if (isset($cmds->mount_vol)) {
        foreach($cmds->mount_vol as $key => $value) { 
            ...
            $table->data[$i][] = '<input type="text" name="'.LTIDS_LTI_VOLUMES_CMD.'name[]"  size="15" value="'.$key.'" readonly style="background-color:#eee;"/>';
            $table->data[$i][] = '<input type="text" name="'.LTIDS_LTI_VOLUMES_CMD.'link[]"  size="30" maxlength="60"  value="'.$value.'" />';
